...
#if defined(NDEBUG)
# error "Coalcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x95894d4bd9d5f03f90db4655d860a6db78bd1d06ab2815aa17deda9367e6c2ac");
...
const string strMessageMagic = "Coalcoin Signed Message:\n";
...
{
    // Coalcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 53 * COIN;  //Lenin's Age of Death
    if (nBestHeight == 0) //M.C make the premine possible, the MAINNET-MAN
    {
        nSubsidy = 900000 * COIN; //2% of premine
    }
    nSubsidy >>= (nHeight / 420000); // Coalcoin: 420k blocks in ~4 years
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 12 * 60 * 60; // Coalcoin: 12 hour
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coalcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coalcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1593369981, nBits=1e0ffff0, nNonce=2084524493, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "21/Aug/1940 Day of the death of Leon Trotsky, Dies at 60"; // Soviet hero
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 900000 * COIN; //450000 coins = 1% of premine
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0483508b3b021317eecece7d8e0ba2bae500afbbe8743afbaaa2552fef2194bcc5a6b6e75b3ca5271f551ab772db15b8486e117240414857cf5d43e567949f0cd5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1593369981;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085267983;

        if (fTestNet)
        {
            block.nTime    = 1593369981;
            block.nNonce   = 385686680;
        }

        // ----- EXTRA CODE BEGIN ----- //
/*
        if (false && block.GetHash() != hashGenesisBlock) //Mine the genesis block hash :)
        {
...
*/
        ...
        assert(block.hashMerkleRoot == uint256("0xf066f9a2f0dda84077ed759054164e612a2c44033b2732ad118cb4e0f575ce60"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xba, 0x0f, 0xc6, 0xbd }; // Coalcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoalcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoalcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CoalcoinMiner(CWallet *pwallet)
{
    printf("CoalcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coalcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CoalcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CoalcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CoalcoinMiner, pwallet));
}
